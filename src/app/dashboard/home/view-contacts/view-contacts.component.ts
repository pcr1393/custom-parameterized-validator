...
import { EscalationsOverlapValidator } from '../../validators/escalations-overlap.validator';
import { EscalationCheckValidator } from '../../validators/escalation-check.validator';
...
export class ViewContactsComponent implements OnInit {
    ...
    this.contactListService.getAllContacts().subscribe(contacts => {
      ...
      for (let i = 0; i < this.contacts.length; i++) {
        this.employeesFormArray.push(this.fb.group({
          ...
          'completed': [this.contacts[i] && this.contacts[i].completed ?
          this.contacts[i].completed : '', [EscalationsOverlapValidator(this.contacts[i].target), EscalationCheckValidator()]]
        }));
      }
    });
  }
  ...
  getFormGroupBasedOnIndex(index: number): FormGroup {
   return this.employeesFormArray.at(index) as FormGroup;
  }
